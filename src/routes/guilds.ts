...
const updateGuildSchema = z.object({
  ...
  ticketSupportRole: z.string().nullable().optional(),
  ticketOpenMessage: z.string().max(2000).nullable().optional(),
  ticketVoice: z.boolean().optional(),
  ticketVoiceCategory: z.string().nullable().optional(),
  // Bot personnalisé
  ...
  // Stats & Logs
  statsChannel: z.string().nullable().optional(),
  memberLogsChannel: z.string().nullable().optional(),
  // DATABASE ID
  databaseIdAdminRole: z.string().nullable().optional(),
});

export async function guildRoutes(app: FastifyInstance) {
  app.addHook('preHandler', requireAuth);

  // GET /api/guilds/:id → config complète + statut freemium
  app.get<{ Params: { id: string } }>(
    ...
    async (request, reply) => {
      ...
      if (!guild) {
        return reply.status(404).send({ error: 'Not Found', message: 'Le bot n\'est pas sur ce serveur.' });
      }
      // Marque l'utilisateur comme licenseHolder s'il n'y en a pas encore
      if (request.user && !guild.licenseHolderId) {
        await claimLicenseHolder(guild.id, request.user.id);
      }
      // Trace la session pour anti-collusion
  ...
  // PATCH /api/guilds/:id → modifie la config (gardée par freemiumPatchGate)
  ...
  // GET /api/guilds/:id/channels → liste des salons Discord (via bot token)
  app.get<{ Params: { id: string } }>(
    '/:id/channels',
    { preHandler: requireGuildAdmin },
    async (request, reply) => {
      try {
        ...
        // Trier : catégories d'abord, puis texte et vocal
        const sorted = channels
          .filter((c) => [0, 2, 4, 5, 15].includes(c.type))
          .sort((a, b) => a.position - b.position);
        return sorted;
      } catch {
        return reply.status(502).send({ error: 'Impossible de récupérer les salons.' });
      }
    },
  );

  // GET /api/guilds/:id/roles → liste des rôles Discord (via bot token)
  app.get<{ Params: { id: string } }>(
    '/:id/roles',
    { preHandler: requireGuildAdmin },
    async (request, reply) => {
      try {
        ...
        return reply.status(502).send({ error: 'Impossible de récupérer les rôles.' });
      }
    },
  );

  // GET /api/guilds/:id/stats → statistiques du serveur
  app.get<{ Params: { id: string } }>(
    '/:id/stats',
    { preHandler: requireGuildAdmin },
    async (request, reply) => {
      try {
        ...
        return reply.status(502).send({ error: 'Impossible de récupérer les statistiques.' });
      }
    },
  );

  // GET /api/guilds/:id/logs → journal des entrées/sorties membres
  app.get<{ Params: { id: string }; Querystring: { page?: string; type?: string } }>(
    '/:id/logs',
    { preHandler: requireGuildAdmin },
    async (request) => {
      const page = Math.max(1, parseInt(request.query.page ?? '1', 10));
      const type = request.query.type; // 'join' | 'leave' | undefined
      const take = 50;
